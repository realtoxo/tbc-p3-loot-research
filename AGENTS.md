...
| `just sim` | Refresh EVERY simulated figure: all profiles at all three boss armor tiers, then the weapon pair rounds, then the pages that read them. Needs the binary and takes tens of minutes, so it sits outside `just regen` and `just check` |
| `just sim-weapons` | Rerun only the weapon pair rounds, every matched pair as a variant of each anchor profile, then rewrite the sim pages that show them. `just sim` runs this too |
...
- **Collect facts. Derive orderings. Judge nothing.** Amended on 22 August 2026 by the guild lead, verbatim in `data/judgments/derived-orderings.yaml`. An agent may emit an ordering that is a PURE FUNCTION of measurement, a delta table sorted best to worst, a denial cost read as this item minus the next best available, because rerunning the pipeline recomputes it and a reader argues with the measurement rather than with a method. What stays out is unchanged: any weighing of quantitative against qualitative inputs, any single number blending things a simulation cannot compare, and who actually receives a drop. Those remain council judgments and live in `data/judgments/`. If a task seems to require weighing facts no measurement compares, say that it is out of scope rather than inventing a method. <!-- why: docs/framework.md -->
...
- **A claim without provenance is a defect, not a shortcut.** State only what a source supports. A number with no source survives rewrites and gets cited later, so it is worse than no number. A claim citing "our own simulation" must name the figure it means, and there are exactly three places figures live. `data/facts/sim-figures.yaml` holds every run at OUR gear anchors, 13 specs across entry, tier and best in slot, each with its `standard_error`. `data/facts/variant-sims.yaml` holds the weapon pair rounds, each pair a variant of an anchor profile and comparable with that anchor's figure. `crit.yaml.simulated_figures` holds four raid-buffed crit percentages produced against the engine's **own** presets rather than ours, single-source and flagged as uncorroborated. Anything else is unsourced, including any price for a set bonus, which is still measured only in the three cases `sim-results.yaml` names. <!-- unenforced -->
...
- **A simulated character has a race, and an illegal race does not error, it deletes every base stat.** `sim/core/base_stats.go` registers base stats per race AND class, so a pairing that does not exist in 2.4.3 returns Go's zero value. `run_sims.py` sent Blood Elf for every spec until 15 August 2026, and Blood Elf cannot be a warrior, a druid or a shaman, so six specs ran with no base strength, agility or stamina and simply measured smaller: the Fury Warrior by 472 DPS and the Arms Warrior by 335. Races are recorded in `roster.yaml::races.by_spec` and both the runner and `just check` reject an illegal pairing. <!-- test: tools/check_sim_profiles.py -->
