...
Seven documents carry the context a newcomer needs before changing anything.
...
- **A claim without provenance is a defect, not a shortcut.** State only what a source supports. A number with no source survives rewrites and gets cited later, so it is worse than no number. ONE simulation has been run: `crit.yaml.simulated_figures` records four raid-buffed crit percentages produced by the wowsims TBC engine against **its own** committed Phase 3 presets, single-source and flagged as uncorroborated. No run against OUR gear anchors exists, `sim-profiles.yaml` records nothing collected, and no set bonus has ever been priced. So a claim citing "our own simulation" still has no source unless it is one of those four figures, and it must name them. <!-- unenforced -->
- **Generated files defend themselves.** `data/facts/drops.csv`, `items.csv`, `item-effects.csv`, `hit-captured.yaml`, `set-stats.yaml` and `transcript-mentions.csv` are produced by the transform, so editing one by hand loses the work on the next `just regen`, and `just check` fails the build by regenerating and diffing. If a generated file is wrong, fix the data or the transform, never the output. <!-- test: justfile::check -->
...
- **A captured set may not wear what its anchor could not reach.** The entry anchor is the set worn before Phase 3, so it holds no Mount Hyjal or Black Temple drop and no Mount Hyjal reputation reward. `tier_hands_only` means Archimonde is not down, so it holds no Archimonde drop. Nothing at any anchor comes from Mother Shahraz, the Illidari Council, Illidan or Arena Season 3. Each was individually plausible while reading one file and impossible against the drop table. <!-- test: tools/check_capture_availability.py -->
